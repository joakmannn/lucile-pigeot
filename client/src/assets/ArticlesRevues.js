        ...
        <>
          <div className='w-full flex justify-center mt-16'>
            {loading ? (
              ...
              <div className='flex flex-col md:flex-row justify-center items-center md:space-y-0 md:space-x-10 w-full max-w-6xl'>
                {posts
                  .filter(post => post.type === 'Article de revue') // Filtrer les articles de type revue
                  .map((post, index) => (
                    <article
                      key={index}
                      className="w-full md:w-1/2 flex flex-col items-start justify-between p-6 border border-gray-200 rounded-lg transition-transform duration-300 hover:shadow-lg hover:scale-105"
                    >
